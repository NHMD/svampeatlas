...
SELECT t._id, t.FullName, dkn.vernacularname_dk, GROUP_CONCAT(t2.FullName ORDER BY t2.RankID ASC SEPARATOR ", ") as Classification, t.RankName 
INTO OUTFILE '/tmp/species_gyldendal.csv'
  FIELDS TERMINATED BY ';' OPTIONALLY ENCLOSED BY '"'
  LINES TERMINATED BY '\n'
FROM Taxon t JOIN Taxon t2 JOIN TaxonAttributes ta ON t._id=ta.taxon_id AND ta.PresentInDK = 1 AND t._id = t.accepted_id AND t.RankID > 9900 AND t.Path LIKE "66576, 60212, 60059%" AND t.Path LIKE CONCAT(t2.Path, '%') LEFT JOIN TaxonDKnames dkn ON t.vernacularname_dk_id=dkn._id  GROUP BY t._id;
...
SELECT t._id, t.FullName, dkn.vernacularname_dk, GROUP_CONCAT(DISTINCT c.Name SEPARATOR ", ") as FruitBodyType, GROUP_CONCAT(DISTINCT t3.FullName ORDER BY t3.RankID ASC SEPARATOR ", ") as Classification,t.RankName, COUNT( distinct tc._id) as species_count 
INTO OUTFILE '/tmp/genera_gyldendal.csv'
  FIELDS TERMINATED BY ';' OPTIONALLY ENCLOSED BY '"'
  LINES TERMINATED BY '\n'
FROM
